...
class NoteToMidi:
    ...
    def __init__(
        ...
        midi_file_path = (
            name + ".mid" if file_path == "" else file_path + "/" + name + ".mid"
        )
        mid.save(midi_file_path)

    @staticmethod
    def create_note(
        note_height: int = 0, note_delay: int = 0, note_time: int = 0
    ) -> (Message, Message):
        """Returns a tuple of 2 MIDI messages, the first one to start a note, and the second to end the note."""

        return Message("note_on", note=note_height, time=note_delay), Message(
            "note_off", note=note_height, time=note_time
        )
    ...
    def add_note_to_track(self):
        ...
        for note in self.note_list:
            ...
            if (
                not isinstance(note[1], int)
                or not isinstance(note[2], int)
                or not isinstance(note[3], int)
            ):
                raise ValueError(
                    f"{note[1]}, {note[2]} and {note[3]} should all be int"
                )

        for i in range(len(self.note_list)):
            height = self.find_height(self.note_list[i][0], self.note_list[i][1])
            self.track.append(
                self.create_note(height, self.note_list[i][2], self.note_list[i][3])[0]
            )
            self.track.append(
                self.create_note(height, self.note_list[i][2], self.note_list[i][3])[1]
            )
